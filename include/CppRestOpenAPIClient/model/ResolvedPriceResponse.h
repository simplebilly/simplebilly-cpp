/**
 * SimpleBilly API
 * Simplebilly API - Bookkeeping, CRM, ERP. Multi-tenant API: a tenant is isolated and routed by subdomain (or a configured custom domain) under the base domain.  ## Rate limiting All endpoints are rate-limited per client IP: **100 requests per minute** on API routes and **5 requests per minute** on authentication routes. Exceeding a limit returns `429 Too Many Requests`; the window resets after 60 seconds.
 *
 * The version of the OpenAPI document: 0.1.0
 * Contact: support@simplebilly.com
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.25.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * ResolvedPriceResponse.h
 *
 * 
 */

#ifndef ORG_OPENAPITOOLS_CLIENT_MODEL_ResolvedPriceResponse_H_
#define ORG_OPENAPITOOLS_CLIENT_MODEL_ResolvedPriceResponse_H_

#include <boost/optional.hpp>

#include "CppRestOpenAPIClient/ModelBase.h"

#include <cpprest/details/basic_types.h>

namespace org {
namespace openapitools {
namespace client {
namespace model {



class  ResolvedPriceResponse
    : public ModelBase
{
public:
    ResolvedPriceResponse();
    virtual ~ResolvedPriceResponse();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;


    /////////////////////////////////////////////
    /// ResolvedPriceResponse members


    /// <summary>
    /// True when no tier matched and the product list price was used.
    /// </summary>
    bool isIsListPrice() const;
    bool isListPriceIsSet() const;
    void unsetIsListPrice();
    void setIsListPrice(bool value);

    /// <summary>
    /// Applied tier, if any matched.
    /// </summary>
    utility::string_t getPriceTierId() const;
    bool priceTierIdIsSet() const;
    void unsetPriceTierId();
    void setPriceTierId(const utility::string_t& value);

    utility::string_t getProductId() const;
    bool productIdIsSet() const;
    void unsetProductId();
    void setProductId(const utility::string_t& value);

    int64_t getQuantity() const;
    bool quantityIsSet() const;
    void unsetQuantity();
    void setQuantity(int64_t value);

    utility::string_t getUnitPrice() const;
    bool unitPriceIsSet() const;
    void unsetUnitPrice();
    void setUnitPrice(const utility::string_t& value);


protected:
    bool m_IsListPrice;
    bool m_IsListPriceIsSet;

    boost::optional<utility::string_t> m_PriceTierId;

    utility::string_t m_ProductId;
    bool m_ProductIdIsSet;

    int64_t m_Quantity;
    bool m_QuantityIsSet;

    utility::string_t m_UnitPrice;
    bool m_UnitPriceIsSet;

};


}
}
}
}

#endif /* ORG_OPENAPITOOLS_CLIENT_MODEL_ResolvedPriceResponse_H_ */
